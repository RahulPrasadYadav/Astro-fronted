    ...
    content: (
      <div className="relative w-full flex items-center justify-center">
        <div className="relative w-80 h-80 md:w-96 md:h-96">
          ...
          <div
            className="absolute inset-0 border-2 border-astro-gold/30 rounded-full animate-spin"
            style={{ animationDuration: "25s" }}
          />
          <div
            className="absolute inset-0 border border-astro-orange/30 rounded-full animate-spin"
          ...
          <div className="relative w-full h-full rounded-full overflow-hidden border-4 border-astro-gold/60 bg-gradient-to-br from-astro-gold/30 to-white/10 animate-float">
            <img
              ...
              alt="Holographic Astrologer"
              className="w-full h-full object-cover opacity-80"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/30 via-transparent to-astro-gold/10" />
            <div className="absolute top-4 right-4 w-14 h-14 rounded-full bg-astro-gold/90 text-2xl flex items-center justify-center animate-bounce">
              ✨
            </div>
          </div>
          {[Sun, Moon, Star, Sparkles].map((Icon, idx) => (
            <div
              key={idx}
              className={`absolute w-10 h-10 rounded-full bg-astro-gold/30 text-astro-navy flex items-center justify-center shadow-lg animate-orbit-${idx}`}
              style={{
                top: "50%",
                left: "50%",
      ...
      <div className="relative w-full flex items-center justify-center">
        <div className="grid grid-cols-3 gap-6">
          {[0, 1, 2].map((i) => (
            <div
              key={i}
              className="w-28 h-44 md:w-36 md:h-56 rounded-xl bg-white/10 border border-astro-gold/40 backdrop-blur-md shadow-lg animate-tilt"
              style={{
  ...
  },
  {
    ...
    content: (
      <div className="relative w-72 h-72 md:w-96 md:h-96">
        <div
          className="absolute inset-0 rounded-full border-2 border-astro-gold/30 animate-spin"
          style={{ animationDuration: "30s" }}
        />
        <div
          className="absolute inset-6 rounded-full border border-astro-gold/20 animate-spin"
        ...
        {[...Array(8)].map((_, i) => (
          <div
            ...
            style={{
              left: "50%",
              top: "50%",
              transform: `translate(-50%, -50%) rotate(${(360 / 8) * i}deg) translateX(140px)`,
            }}
          />
        ))}
      </div>
    ...
    title: (
      <>
        ...
        <span className="text-transparent bg-clip-text bg-gradient-to-r from-astro-gold to-astro-orange">
          Golden Cosmos
        </span>
      </>
    ),
                ...
                <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                  ...
                  <div className="mt-10 space-y-4">
                    ...
                    <div className="flex gap-4 justify-center pt-2">
                      ...
                      <Button
                        variant="outline"
                        className="border-2 border-astro-gold text-astro-gold hover:bg-astro-gold hover:text-astro-navy"
                      >
                        Learn More
                      </Button>
                    </div>
          ...
          {Slides.map((_, i) => (
            <button
              ...
              className={`w-2.5 h-2.5 rounded-full transition-all ${
                i === index
                  ? "bg-astro-gold scale-125 shadow"
